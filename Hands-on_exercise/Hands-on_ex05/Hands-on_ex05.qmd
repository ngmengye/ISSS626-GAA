...
If development are evenly distributed geographically?
No -> is there sign of spatial clustering?
Yes -> where are these clusters?
...
equal interval = equal range of values quantile = equal number of observations
...
s0: sum of all weights in the spatial weights matrix s1: sum of squared weights for each region's links. rows sums s2: sum of squared weights of all the elements in the weights matrix
...
Why Calculate This Mean?
Calculating the mean of the simulated Moran's I values helps to understand the distribution of Moran's I under the null hypothesis of spatial randomness. It gives you an idea of what the Moran's I would typically be if there were no spatial autocorrelation present in the data.
